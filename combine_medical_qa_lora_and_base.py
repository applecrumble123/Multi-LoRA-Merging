...
medical_qa_lora_path = os.path.join(root_code_folder, "Medical QA Weights/medical_qa_lora_model.pt")
combined_model_with_filtered_medical_qa_lora_path = os.path.join(root_code_folder, "Medical QA Weights/combined_model_with_filtered_medical_qa_lora.pt")

combined_medical_qa_lora_param_difference_file_path = os.path.join(root_code_folder, "Medical QA Weights/combined_medical_qa_lora_param_difference.txt")

# base_model_weights = torch.load(base_model_weights_path, map_location=device)
medical_qa_lora_weights = torch.load(medical_qa_lora_path, map_location=device)
...
filtered_medical_qa_lora_weights = torch.load(medical_qa_lora_path, map_location=device)

# print("Keys in filtered_medical_qa_lora_weights:")
# for key in filtered_medical_qa_lora_weights.keys():
#     print(key)
...
base_model_weights_medical_qa = torch.load(base_model_weights_path, map_location=device)

combine_lora_and_base(base_model_weights_medical_qa, filtered_medical_qa_lora_weights, combined_model_with_filtered_medical_qa_lora_path, alpha=256, rank=128,save = True)
combined_model_with_filtered_medical_qa_lora_weights = torch.load(combined_model_with_filtered_medical_qa_lora_path, map_location=device)
...
compare_model_parameters_with_percentage_to_file(
    ...
    lora_model=combined_model_with_filtered_medical_qa_lora_weights,
    output_file=combined_medical_qa_lora_param_difference_file_path
)
